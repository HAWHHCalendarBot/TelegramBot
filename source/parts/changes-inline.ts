...
function generateInlineQueryResultFromChange(change: Change, from: User) {
...
bot.action(/^c:a:(.+)#(.+)#(.+)$/, preAddMiddleware, async context => {
	...
	if (myChangeToThisEvent.length > 0) {
		...
		await context.telegram.sendMessage(context.from!.id, text, {reply_markup: keyboardMarkup, parse_mode: 'Markdown'})
		return
	}
