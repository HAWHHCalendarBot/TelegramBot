...
import * as calendarHelper from '../lib/calendar-helper'

function getUrl(context: MyContext): string {
	return calendarHelper.getUrl(context.from!.id, context.state.userconfig)
}

export const menu = new TelegrafInlineMenu(mainText as any)

menu.submenu('🍏 iOS / macOS', 'apple', new TelegrafInlineMenu(appleText))
	.urlButton('Kalender abonnieren', ctx => `https://calendarbot.hawhh.de/ics.html?url=${getUrl(ctx as any)}`)

menu.submenu('🗂 HAW Mailer (Exchange)', 'exchange', new TelegrafInlineMenu(exchangeText as any))
...
menu.submenu('🍰 Google Kalender', 'google', new TelegrafInlineMenu(googleText as any))
...
menu.submenu('Freestyle 😎', 'freestyle', new TelegrafInlineMenu(freestyleText as any))
	.urlButton('Kalender abonnieren', ctx => `https://calendarbot.hawhh.de/ics.html?url=${getUrl(ctx as any)}`)

function mainText(context: MyContext): string {
...
function suffixText(context: MyContext): string {
	const {calendarfileSuffix} = context.state.userconfig
...
const suffixMenu = new TelegrafInlineMenu(suffixText as any)
menu.submenu(suffixButtonText as any, 'suffix', suffixMenu)

suffixMenu.button('Generieren…', 'g', {
	doFunc: async ctx => {
		// 10^8 -> 10 ** 8
		const fromTime = Date.now() % (10 ** 8)
		await setSuffix(ctx as any, String(fromTime))
	}
})

suffixMenu.question('Manuell setzen…', 's', {
	setFunc: setSuffix as any,
...
function exchangeText(context: MyContext): string {
...
function googleText(context: MyContext): string {
...
function freestyleText(context: MyContext): string {
